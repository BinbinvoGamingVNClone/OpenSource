...
local redzlib = loadstring(game:HttpGet("https://raw.githubusercontent.com/BinbinvoGamingVNClone/RedzLibV5/refs/heads/main/RedzLibV5.lua"))()

local Window = redzlib:MakeWindow({
  Title = "Shinichi Hub | Blox Fruits",
  SubTitle = "by BinbinvoGaming",
  SaveFolder = "BloxFruitsShinichiHub"
})

local I = Window:MakeTab({"Info", "info"})
local Mn = Window:MakeTab({"Home", "home"})
local M = Window:MakeTab({"Automatic", "code"})
local P = Window:MakeTab({"Player", "user"})
local S = Window:MakeTab({"Shop", "store"})
local V4 = Window:MakeTab({"Race V4", "disc"})
local V = Window:MakeTab({"Visuals", "computer"})
local Ve = V
local E = M
local St = Window:MakeTab({"Settings", "Settings"})

I:DiscordInvite({
    Name = "Discord Nhà Phát Triển",
    Logo = "rbxassetid://138689078158833",
    Description = "Vào Để Nhận Những Cập Nhật Mới",
    Invite = "https://discord.com/invite/wWX6EAYzrA"
  })
